   ...
   if (error) return <p>Error: {error}</p>;
    ...
    return (
      <div className="p-6 font-serif lg:flex lg:justify-center">
        {orderedData &&
          isData(orderedData) &&
          getSortedData(orderedData).map((item: any) => (
            <div id="print-section">
              <div className="mb-2">
                <div className="flex flex-row justify-between">
                  <h2 className="text-red-500 font-bold text-2xl font-serif mb-2">
                    Order Details
                  </h2>
                  <img
                    id="hiddenClass"
                    src="assets/print.svg"
                    className="w-6 h-6 cursor-pointer hiddenClass"
                    alt="Print"
                    onClick={() => {
                      print();
                    }}
                ...
                <table className="table-auto text-left w-full border-separate font-serif">
                  ...
                  <tbody className="text-xl">
                    {item?.orderItems?.map((item: any) => (
                      <tr key={item.name}>
                        <td className="font-mono max-w-[150px] lg:max-w-full text-lg truncate overflow-hidden">
                          {item?.name}
                        </td>
                        ...
                        <td className="text-right text-lg tracking-[1px]">
                ...
                <div className="border-t-2 mt-2 pt-2 border-red-500">
                  <div className="text-lg flex justify-between">
                    ...
                    <div className="flex flex-col gap-2">
                      ...
                      <div className="text-right tracking-[1px]">
                        {getTaxAmount(item).toFixed(2)}
                      </div>
                      <div className="text-right font-bold tracking-[1px]">
                        {getTaxableAmount(item).toFixed(2)}
                      </div>
                    </div>
                  </div>
                </div>
              </div>
              <div className="flex flex-col mt-6 justify-center lg:text-xl">
                <h2 className="w-[1/2]">Amount (in words): </h2>
                <p className="w-[1/2] capitalize text-red-500">{getWordAmount(item)} </p>
              </div>

              <div id="hiddenClass" className="mt-6">
                <h2
                ...
                <div className="flex flex-col gap-4 justify-center mt-4">
                  <div id="hiddenClass" className="flex justify-center">
                    <button
                      onClick={() => {
              ...
              {showBill && (
                <div className="mt-4">
                  Each of the <span className="text-red-500 text-lg font-bold tracking-[1px]">{noOfPeople} {" "}</span> 
                  individuals will contribute <span className="text-red-500 text-lg font-bold tracking-[1px]">{splittedAmount.toFixed(2)}</span>, 
                  bringing the total to <span className="text-red-500 text-lg font-bold tracking-[1px]">{getTaxableAmount(item).toFixed(2)}</span>.
                </div>
              )}
            </div>
          ))}
        ...
        {orderedData && !isData(orderedData) && (
          <div className="flex flex-col items-center justify-center h-[80vh]">
            ...
            <div className="flex flex-row mt-6 gap-2 items-center justify-center">
              <button
              ...
              >
                <div className="flex items-center justify-center">
                  <img src="assets/menu.svg" className="w-6 h-6" alt="Menu" />
